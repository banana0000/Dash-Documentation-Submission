...
def test_the_tag_dash_emits_follows_the_forwarded_scheme(app):
    ...
    the scheme (the boilerplate found this at its 1.6.34 floor bump: green
    on 2.7.1, red on 2.8.0, on every Flask leg; this fork's `>=2.3.4` floor
    resolved to 2.10.0 in CI and hit the same wall). Either lane can be the
    one you did not mean to test; the browser one is the one this tag
    lives in.
    """
