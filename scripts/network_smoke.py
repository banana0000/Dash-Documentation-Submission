...
SITE_H1 = "# Dash Showcases — live playgrounds for 2plot.ai components"
...
def satellite_checks(base: str) -> None:
    ...
    def page_llms_nav():
        status, _, text = get("/color-picker/llms.txt")
        expect(status == 200, f"/color-picker/llms.txt {status}")
    ...
    def agents_and_browsers_get_different_types():
        ...
        status, md_headers, md = get("/color-picker/llms.txt")
        expect(status == 200, f"/color-picker/llms.txt {status}")
