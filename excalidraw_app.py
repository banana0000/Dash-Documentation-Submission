"""Standalone example for the dash-excalidraw component.
Install with: pip install "git+https://github.com/pip-install-python/dash-excalidraw.git"
(this is newer than the 0.0.4 release on PyPI -- see the command prop note
below for why that matters). Run: python app.py, then open
http://127.0.0.1:8060
...
On the package: the 0.0.4 release on PyPI only reads `initialData` once at
...
viewBackgroundColor. This session used to work around that with a local
patch to its minified bundle plus a "remount with a new id" trick, both
gone now. The 0.1.0 build from GitHub instead exposes a `command` prop
(`{"id": ..., "type": "updateScene", "payload": {...}}`) that updates the
mounted scene imperatively without remounting or losing what's on the
canvas -- so a future live-editing feature (an "apply to selection" tool,
for instance) should use that, not the old approach.
"""
...
def _connector(arrow_id, points, color, opacity, seed, label=None, label_pos=None,
    ...
    if label:
        lx, ly = label_pos
        elements.append(_text(f"{arrow_id}-label", group_id, lx, ly, 60, 20, 14, "#475569", label,
                               opacity, seed + 1))
    return elements
...
def flowchart_sample(origin_x, origin_y, opacity):
    ...
    elements += _connector("flow-arrow-start", [(sx, s_bot), (sx, s1_top)], "#475569", opacity, seed + 100,
                            start_id="flow-start", end_id="flow-step1")
    elements += _connector("flow-arrow-step1", [(sx, s1_bot), (sx, d_top)], "#475569", opacity, seed + 110,
                            start_id="flow-step1", end_id="flow-decision")
    elements += _connector("flow-arrow-yes", [(d_right_x, d_right_y), (y_left_x, y_left_y)],
                            "#10b981", opacity, seed + 120, "Yes", (d_right_x + 20, d_right_y - 22),
                            start_id="flow-decision", end_id="flow-step-yes")
    elements += _connector("flow-arrow-done", [(yx, y_bot), (dnx, dn_top)], "#475569", opacity, seed + 130,
                            start_id="flow-step-yes", end_id="flow-done")
    elements += _connector("flow-arrow-no", [(sx, d_bot), (nx, n_top)], "#ef4444", opacity, seed + 140,
                            "No", (sx + 12, d_bot + 10), start_id="flow-decision", end_id="flow-step-no")
    elements += _connector("flow-arrow-error", [(nx, n_bot), (ex, e_top)], "#475569", opacity, seed + 150,
...
def build_canvas_elements(opacity):
    """The Full HD artboard, plus a sample KPI dashboard -- a starting
    mockup left on the canvas so it isn't a blank artboard."""
    return artboard_frame(opacity) + dashboard_sample(100, 100, opacity)


def build_excalidraw_with_elements(component_id, elements, opacity):
...
def build_excalidraw(component_id, opacity):
    return build_excalidraw_with_elements(component_id, build_canvas_elements(opacity), opacity)
